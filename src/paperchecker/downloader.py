...
def _download_via_libgen(
    ...
    for mirror in ["libgen.is", "libgen.li", "libgen.st"]:
        search_url = f"https://{mirror}/index.php?req={quote(query)}&column=def"
        try:
            req = Request(
                search_url,
                headers={
        ...
        # Extract edition IDs from results table
        edition_ids = re.findall(r'edition\.php\?id=(\d+)', html)

        for edition_id in edition_ids[:3]:
            ed_url = f"https://{mirror}/edition.php?id={edition_id}"
            try:
                ed_req = Request(
                    ed_url,
                    headers={
                ...
                ed_resp = urlopen(ed_req, timeout=15)
                ed_html = ed_resp.read().decode("utf-8", errors="replace")
            except Exception:
                continue

            # Find MD5 hash on the edition page
            md5 = re.search(r'md5=([a-f0-9]{32})', ed_html)
            if md5:
                path = _download_libgen_md5(mirror, ed_html, output_dir)
                if path:
                    return path


def _download_libgen_md5(
    mirror: str, ed_html: str, output_dir: str
) -> str | None:
    """Download a file from libgen edition page via IPFS/Cloudflare links."""
    from urllib.request import urlopen, Request
    import re
    ...
    for pattern in [
        ...
        r'https?://dweb\.link/ipfs/[a-zA-Z0-9]+[^"\s]*',
    ]:
        m = re.search(pattern, ed_html)
        if m:
            pdf_url = m.group(0).rstrip('"')
            # Extract filename from URL if present
            fn_match = re.search(r'filename=([^&"\s]+)', pdf_url)
            if fn_match:
                filename = fn_match.group(1)
            else:
                md5 = re.search(r'([a-f0-9]{32})', ed_html)
                filename = _safe_filename(f"libgen_{md5.group(1) if md5 else 'unknown'}") + ".pdf"
            return _download_via_http(pdf_url, output_dir, filename, "libgen")
